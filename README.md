...
- Stat-arb price-history export: `python scripts/export_stat_arb_price_history.py --source-mode massive_flatfiles --output <file>` (builds validated training JSON from Massive flat files plus corporate actions; use `providers` only as the fallback path)
